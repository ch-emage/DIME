...
import dime_v2.anomaly_engine.models
import dime_v2.anomaly_engine.core.core_utils as anomaly_engine_core_utils
import dime_v2.anomaly_engine.selectors.feature_selector
import dime_v2.anomaly_engine.preprocessing.feature_processor
import dime_v2.anomaly_engine.preprocessing.feature_processor as anomaly_engine_feature_processor
import dime_v2.anomaly_engine.selectors.feature_selector as anomaly_engine_feature_selector
import dime_v2.anomaly_engine.models.network_models as anomaly_engine_network_models
from .motion_utils import MotionProcessor  # New import
from dime_v2.anomaly_engine.core.feature_utils import FeatureSlicer as FeatureSlicer_obj
...
class AnomalyNet(torch.nn.Module):
    ...
    def load(
        ...
        featuresampler=anomaly_engine_feature_selector.IdentitySelector(),
        ...
        **kwargs,
        ...
        self.feature_extractor = anomaly_engine_core_utils.FeatureExtractor(
            self.backbone, self.layers_to_extract_from, self.device
        )
        # self.feature_extractor = anomaly_engine_core_utils.FeatureExtractorTRT10(
        #     model_path=r"D:\25-DIME\wideresnet101_fp16.engine", 
        #     layers_to_extract_from=self.layers_to_extract_from
        # )
        feature_dimensions = self.feature_extractor.feature_dimensions(input_shape)
        self.forward_modules["feature_extractor"] = self.feature_extractor
        
        preprocessing = anomaly_engine_feature_processor.Preprocessing(
            feature_dimensions, pretrain_embed_dimension
        )
        self.forward_modules["preprocessing"] = preprocessing
        self.forward_modules["enhancer"] = anomaly_engine_feature_processor.FeatureEnhancer()
        
        self.target_embed_dimension = target_embed_dimension
        preadapt_aggregator = anomaly_engine_feature_processor.Aggregator(
    ...
    def _embed(self, images, detach=True, provide_patch_shapes=False):
        ...
        with torch.no_grad():
            ...
            print("[FeatureExtractTime]", (t2-t1)*1000,"ms")
    ...
    def load_from_path(self, load_path: str, device: torch.device, nn_method: anomaly_engine_core_utils.ProximitySearcher(False, 4), prepend: str = "") -> None:
        ...
        anomaly_params["backbone"] = anomaly_engine_network_models.load(anomaly_params["backbone.name"])
        ...
        if sequence_config.get("enable", False) and os.path.exists(os.path.join(load_path, prepend + "sequence_model.pth")):
            sequence_state = torch.load(os.path.join(load_path, prepend + "sequence_model.pth"), weights_only=True)
